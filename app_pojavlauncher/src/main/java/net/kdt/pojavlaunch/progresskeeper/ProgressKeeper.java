...
public class ProgressKeeper {
    ...
    private static final HashMap<String, Runnable> sCancellationHandlers = new HashMap<>();
    ...
    public static synchronized void setCancellationHandler(String progressRecord, Runnable handler) {
        if(handler == null) sCancellationHandlers.remove(progressRecord);
        else sCancellationHandlers.put(progressRecord, handler);
    }

    public static synchronized boolean hasCancellationHandler(String progressRecord) {
        return sCancellationHandlers.containsKey(progressRecord);
    }

    public static void cancel(String progressRecord) {
        Runnable handler;
        synchronized (ProgressKeeper.class) {
            handler = sCancellationHandlers.get(progressRecord);
        }
