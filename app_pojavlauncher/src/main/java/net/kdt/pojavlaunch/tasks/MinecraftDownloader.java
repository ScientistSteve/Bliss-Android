...
public class MinecraftDownloader {
    ...
    private static final String NATIVES_EXTRACTED_MARKER = ".extracted";
    private static volatile MinecraftDownloader sActiveDownloader;
    ...
    private volatile Thread mDownloaderThread;
    private volatile ThreadPoolExecutor mDownloaderPool;
    ...
    public void start(@Nullable Activity activity, @Nullable JMinecraftVersionList.Version version,
        ...
        mCancelled = false;
        sActiveDownloader = this;
        ProgressKeeper.setCancellationHandler(ProgressLayout.DOWNLOAD_MINECRAFT, this::cancelDownload);
        sExecutorService.execute(() -> {
            mDownloaderThread = Thread.currentThread();
            try {
                if(!isOnline) {
                    ensureVersionMetadataInstalled(realVersion);
                } else {
                ...
                if(sActiveDownloader == this) sActiveDownloader = null;
                if(mDownloaderThread == Thread.currentThread()) mDownloaderThread = null;
                ProgressKeeper.setCancellationHandler(ProgressLayout.DOWNLOAD_MINECRAFT, null);
    ...
    public static void cancelCurrentDownload() {
        MinecraftDownloader downloader = sActiveDownloader;
    ...
    private void cancelDownload() {
        mCancelled = true;
        ProgressKeeper.setCancellationHandler(ProgressLayout.DOWNLOAD_MINECRAFT, null);
        ProgressLayout.setProgress(ProgressLayout.DOWNLOAD_MINECRAFT, 0, "Cancelling...");
        ThreadPoolExecutor downloaderPool = mDownloaderPool;
        if(downloaderPool != null) downloaderPool.shutdownNow();
        Thread downloaderThread = mDownloaderThread;
        if(downloaderThread != null) downloaderThread.interrupt();
    }

    private void throwIfCancelled() {
        if(mCancelled || Thread.currentThread().isInterrupted()) throw new CancellationException("Minecraft download cancelled");
    }

    private void ensureVersionMetadataInstalled(String realVersion) throws IOException {
        throwIfCancelled();
        ...
        File providedJsonFile = createGameJsonPath(realVersion);

        // Check if file exists before trying to read it
        if (!providedJsonFile.exists()) {
            throw new FileNotFoundException("Minecraft " + versionMessage + " is not currently installed. Please connect to the internet to download it.");
        }

        if (providedJsonFile.length() == 0) {
            throw new IOException("Minecraft " + versionMessage + " is needed by " + realVersion);
        }
        ...
        if (providedJson != null && Tools.isValidString(providedJson.inheritsFrom)) {
            ...
            File vanillaJsonFile = createGameJsonPath(providedJson.inheritsFrom);
    ...
    private void downloadGame(Activity activity, JMinecraftVersionList.Version verInfo, String versionName) throws Exception {
        ...
        mDownloaderPool = downloaderPool;
        ...
        try {
            ...
            throw new CancellationException("Minecraft download cancelled");
        } finally {
            if(mDownloaderPool == downloaderPool) mDownloaderPool = null;
        }
    }
    ...
    private void extractNatives(String versionName) throws IOException {
        ...
        String nativesFingerprint = getNativesFingerprint();
        File markerFile = new File(targetDirectory, NATIVES_EXTRACTED_MARKER);
        if(isNativesExtractionCached(markerFile, nativesFingerprint)) return;
        ...
        Tools.write(markerFile.getAbsolutePath(), nativesFingerprint);
    }

    private boolean isNativesExtractionCached(File markerFile, String nativesFingerprint) {
        try {
            File[] files = markerFile.getParentFile().listFiles();
            return markerFile.isFile() && files != null && files.length > 1 &&
                    nativesFingerprint.equals(Tools.read(markerFile));
        }catch (IOException e) {
            return false;
        }
    }

    private String getNativesFingerprint() {
        StringBuilder stringBuilder = new StringBuilder();
        for(File nativeFile : mDeclaredNatives) {
            stringBuilder.append(nativeFile.getAbsolutePath());
            stringBuilder.append(':');
            stringBuilder.append(nativeFile.length());
            stringBuilder.append(':');
            stringBuilder.append(nativeFile.lastModified());
            stringBuilder.append('\n');
        }
    ...
    private boolean scheduleDownload(File targetFile, int downloadClass, String url, String sha1,
                                  long size, boolean skipIfFailed) throws IOException {
        if(isDownloadAlreadySatisfied(targetFile, sha1, size)) return false;
        FileUtils.ensureParentDirectory(targetFile);
        mTotalFileCount++;
