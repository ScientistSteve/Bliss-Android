...
import static net.kdt.pojavlaunch.prefs.LauncherPreferences.PREF_FORCE_RUSSIAN;
...
public class LocaleUtils extends ContextWrapper {
    ...
    public static ContextWrapper setLocale(Context context) {
        if (DEFAULT_PREF == null) {
            ...
            // So only initialize PREF_FORCE_ENGLISH/PREF_FORCE_RUSSIAN for the check below.
            PREF_FORCE_ENGLISH = DEFAULT_PREF.getBoolean("force_english", false);
            PREF_FORCE_RUSSIAN = DEFAULT_PREF.getBoolean("force_russian", false);
        }

        Locale forcedLocale = null;
        if(PREF_FORCE_RUSSIAN) forcedLocale = new Locale("ru");
        else if(PREF_FORCE_ENGLISH) forcedLocale = Locale.ENGLISH;

        if(forcedLocale != null){
            Resources resources = context.getResources();
            Configuration configuration = resources.getConfiguration();

            configuration.setLocale(forcedLocale);
            Locale.setDefault(forcedLocale);
            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.N){
                LocaleList localeList = new LocaleList(forcedLocale);
