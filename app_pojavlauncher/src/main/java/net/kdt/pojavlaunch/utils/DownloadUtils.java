...
public class DownloadUtils {
    ...
    public static void downloadFileMonitored(String urlInput, File outputFile, @Nullable byte[] buffer,
        ...
        for(int attempt = 0; attempt < DOWNLOAD_ATTEMPTS; attempt++) {
            try {
                throwIfDownloadCancelled(monitor);
                ...
                lastException = e;
                Log.w("DownloadUtils", "Download attempt " + (attempt + 1) + " failed for " + urlInput, e);
                if(outputFile.exists()) outputFile.delete();
            }
        }
        throw new IOException("Unable to download from " + urlInput, lastException);
    }
    ...
    private static void downloadFileMonitoredOnce(String urlInput, File outputFile, @Nullable byte[] buffer,
        ...
        conn.setReadTimeout(TIME_OUT);
        conn.setRequestProperty("User-Agent", USER_AGENT);
        if(monitor != null) monitor.onDownloadConnectionOpened(conn);
        try {
            int responseCode = conn.getResponseCode();
            if(responseCode == HttpURLConnection.HTTP_NOT_FOUND) {
                throw new FileNotFoundException(urlInput);
            }
            ...
            IOUtils.closeQuietly(readStr);
            if(monitor != null) monitor.onDownloadConnectionClosed(conn);
            conn.disconnect();
        }
    }

    private static void throwIfDownloadCancelled(Tools.DownloaderFeedback monitor) {
        if(monitor != null && monitor.isCancelled()) throw new CancellationException("Download cancelled");
    }
